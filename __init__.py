...
class Pusher():
    ...
    class Channel():
        ...
        def trigger(self, event, data={}):
            ...
            pusher_qs_sorted = pusher_qs.items()
            pusher_qs_sorted.sort( key=lambda a:a[1], reverse=True )
            pusher_qs_sorted.sort( key=lambda a:a[0] )
            
            # Concatenate keys, values
            pusher_qs_joined = '&'.join([str(i[0]) + '=' + str(i[1]) for i in pusher_qs_sorted])
